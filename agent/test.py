...
class DDQNAgent:
    ...
    def train(self, num_episode: int, plotting_interval=10):
        self.is_test = False
        losses = []
        scores = []
            ...
            while not done:
                ...
                if len(self.replay_buffer) >= self.batch_size:
                    ...
                    non_final_next_states = torch.cat(
                        [torch.tensor(s, dtype=torch.float32, device=device).view(1, -1) for s in batch.next_state if
                         s is not None])
...
#                     print("@@@\n", self.qnetwork_local(non_final_next_states))
...
#                     print(self.qnetwork_target(non_final_next_states).size())  # 32*6
                    Q_targets_next[non_final_mask] = self.qnetwork_target(non_final_next_states).gather(1,
...
#                     print("Q_expected", Q_expected.size())
                    ...
                    # Compute loss
                    loss = F.mse_loss(Q_expected, Q_targets.unsqueeze(1))#.unsqueeze(1)
